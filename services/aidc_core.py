...
def vytvor_qr_obrazek(text, error_correction=qrcode.constants.ERROR_CORRECT_M, format_vystupu='png', vodoznak_text=None, inverze=False, spodni_text=None):
    ...
    if format_vystupu == 'svg' and not vodoznak_text and not spodni_text:
        ...
        qr = qrcode.QRCode(version=None, error_correction=error_correction, box_size=10, border=4, image_factory=factory)
        qr.add_data(text)
        qr.make(fit=True)
        img_io = io.BytesIO()
        qr.make_image().save(img_io)
        img_io.seek(0)
        return send_file(img_io, mimetype='image/svg+xml', as_attachment=True, download_name='matrix.svg')
    ...
    if vodoznak_text:
        ...
        draw.text((w // 2, h // 2), vodoznak_text, fill=fill_col, font=font, anchor="mm")
    ...
    if spodni_text:
        ...
        for radek in radky:
            draw.text((img.width // 2, y_text), radek, fill=fill_col, font=font, anchor="mm")
            y_text += 22
        img = nove_platno
    ...
    img.save(img_io, 'PNG')
    img_io.seek(0)
    return send_file(img_io, mimetype='image/png', as_attachment=False, download_name='matrix.png')

def vytvor_1d_kod(text, typ='code128', format_vystupu='png'):
    try:
        if typ in ['ean13', 'upca']:
            text = re.sub(r'\D', '', text)
            if typ == 'ean13': text = text[:12].zfill(12)
            if typ == 'upca': text = text[:11].zfill(11)
    ...
    return send_file(img_io, mimetype=mimetype, as_attachment=(format_vystupu=='svg'), download_name=f'barcode_{typ}.{format_vystupu}')
...
def vytvor_qr_edukacni(text):
    ...
    img = Image.new('RGB', ((velikost + border * 2) * box, (velikost + border * 2) * box), '#FFFFFF')
    draw = ImageDraw.Draw(img)
    for r in range(velikost):
        for c in range(velikost):
            if not matrix[r][c]: continue
            color = "#333333"
            if (r < 7 and c < 7) or (r < 7 and c >= velikost - 7) or (r >= velikost - 7 and c < 7): color = "#E63946"
            elif r == 6 or c == 6: color = "#1D3557"
            elif (r <= 8 and c == 8) or (r == 8 and c <= 8) or (r == 8 and c >= velikost - 8) or (r >= velikost - 8 and c == 8): color = "#2A9D8F"
            elif (r > 10 and c > 10 and r < velikost - 5 and c < velikost - 5) and (r % 28 == 22 or c % 28 == 22) and qr.version > 1: color = "#9B5DE5"
