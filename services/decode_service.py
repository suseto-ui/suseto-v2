...
def try_hex(payload):
    try:
        ...
        if len(p) % 2 or not re.fullmatch(r"[0-9A-Fa-f]+", p):
            return None
        raw = bytes.fromhex(p)
        ...
        return {
            "type": "hex->utf8",
            "output": txt,
            "confidence": 0.72 + 0.2 * _ascii_ratio(txt),
            "reason": "Platny HEX vstup a dekodovatelny text.",
        }
...
def try_b64(payload):
    try:
        p = str(payload).strip()
        if not re.fullmatch(r"[A-Za-z0-9+/=_-]{8,}", p):
            return None
        for alt in (False, True):
            try:
                ...
                return {
                    "type": "base64->utf8",
                    "output": txt,
                    "confidence": 0.68 + 0.25 * _ascii_ratio(txt),
                    "reason": "Platny Base64 vstup a citelny text.",
                }
...
def try_url(payload):
    try:
        ...
        return {
            "type": "url-decode",
            "output": out,
            "confidence": 0.6 + 0.2 * _ascii_ratio(out),
            "reason": "Obsahuje URL escape sekvence.",
        }
    except Exception as e:
...
def classify(payload):
    try:
        ...
        if p.startswith(("http://", "https://")):
            patterns.append({"name": "URL", "detail": "Web adresa", "confidence": 0.95})
        if p.startswith("WIFI:"):
            patterns.append(
                {"name": "WIFI", "detail": "Wi-Fi QR payload", "confidence": 0.96}
            )
        if p.count(".") == 2:
            patterns.append(
                {
                    "name": "JWT-like",
                    "detail": "Tri segmenty oddelene teckou",
                    "confidence": 0.82,
                }
            )
        if re.fullmatch(r"\d{8}|\d{12}|\d{13}|\d{14}", p):
            patterns.append(
                {
                    "name": "GTIN-like",
                    "detail": "Ciselna delka odpovida GTIN",
                    "confidence": 0.8,
                }
            )
        if re.fullmatch(r"[0-9A-Fa-f\s]+", p) and len(re.sub(r"\s+", "", p)) >= 8:
            patterns.append(
                {
                    "name": "HEX-like",
                    "detail": "Hexadecimalni abeceda",
                    "confidence": 0.78,
                }
            )
        if re.search(r"\(01\)|\(10\)|\(17\)|\(21\)", p):
            patterns.append(
                {
                    "name": "GS1-like",
                    "detail": "Obsahuje bezne AI segmenty",
                    "confidence": 0.84,
                }
            )
        return patterns
...
def chain(payload):
    try:
        ...
        logger.error(
            "chain critical error for payload=%r: %s",
            str(payload)[:80],
            e,
            exc_info=True,
        )
...
def pattern_library(payloads):
    try:
                ...
                rows.append(
                    {
                        ...
                        "patterns": [x["name"] for x in best.get("patterns", [])],
                    }
                )
            except Exception as e:
                logger.warning(
                    "pattern_library item error payload=%r: %s", str(p)[:40], e
                )
        ...
        return {
            "rows": rows,
            "groups": [
                {"pattern_set": k, "count": len(v), "examples": v[:5]}
